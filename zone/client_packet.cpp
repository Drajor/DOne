...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	dbaw->AddQuery(1, &query, MakeAnyLenString(&query,
		"SELECT status,name,lsaccount_id,gmspeed,hideme,time_creation FROM account WHERE id=%i",
		account_id));
	...
	dbaw->AddQuery(3, &query, MakeAnyLenString(&query,
		...
		character_id));
	if (!(mDBAsyncWorkID = dbasync->AddWork(&dbaw))) {
		safe_delete(dbaw);
...
bool Client::FinishConnState2(DBAsyncWork* dbaw) {
	...
	for (i=1; i<=3; i++) {
		...
		if (dbaq->QPT() == 1) {
			database.GetAccountInfoForLogin_result(result, 0, account_name, &lsaccountid, &gmspeed, &gmhideme, &account_creation);
			if(gmhideme)
			{
				trackable = false;
			}
